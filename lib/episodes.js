...
export async function fetchEpisodeList(malId, totalEpisodes) {
  ...
  try {
    const pages = Math.ceil((totalEpisodes || 1) / 100);
    const all   = [];

    for (let p = 1; p <= Math.min(pages, 5); p++) {
      await delay(350);
      const res  = await fetch(`${JIKAN}/anime/${malId}/episodes?page=${p}`);
      if (!res.ok) break;
...
export function getEmbedSources(anilistId, malId, epNumber) {
  ...
  // ── 1. VidNest — الأفضل حالياً، عدة سيرفرات داخلية ──────────────────────
  if (anilistId) {
    // المصدر الرئيسي (gogoanime/hianime)
    sources.push({
      name:     'VidNest',
      label:    'VidNest — Sub',
    ...
    // AnimePahe — جودة أفضل أحياناً
    sources.push({
      name:     'VidNest Pahe',
      label:    'VidNest Pahe — Sub',
  ...
  if (anilistId) {
    ...
    sources.push({
      name:     'MegaPlay Dub',
      label:    'MegaPlay — Dub',
      url:      `https://megaplay.buzz/stream/ani/${anilistId}/${epNumber}/dub`,
      lang:     'dub',
      provider: 'megaplay',
    });
  }

  // ── 3. MegaPlay بـ MAL ID ─────────────────────────────────────────────────
  if (malId) {
    sources.push({
      name:     'MegaPlay MAL',
      label:    'MegaPlay MAL — Sub',
      url:      `https://megaplay.buzz/stream/mal/${malId}/${epNumber}/sub`,
      lang:     'sub',
      provider: 'megaplay',
    });
  }
