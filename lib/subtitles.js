...
//  مع كاش بـ localStorage عشان ما نترجم نفس الحلقة مرتين
// ══════════════════════════════════════════════════════════════════════════════

const TRANSLATE_CACHE_PREFIX = 'subTranslateCache:';
const TRANSLATE_CACHE_MAX    = 30; // أقصى عدد حلقات نخزنها (حماية من امتلاء localStorage)

// مفتاح فريد بسيط يعتمد على عدد السطور + أول/آخر سطر (كافي للتمييز بدون تعقيد)
function getCacheKey(cues) {
  if (!cues?.length) return null;
  const first = cues[0]?.text?.slice(0, 30) || '';
  const last  = cues[cues.length - 1]?.text?.slice(0, 30) || '';
  return `${TRANSLATE_CACHE_PREFIX}${cues.length}:${first}:${last}`;
}
...
function writeCache(key, cues) {
  try {
    localStorage.setItem(key, JSON.stringify(cues));
    pruneOldCacheEntries();
  } catch {
    // localStorage ممتلئ أو غير متاح — نتجاهل بصمت، الترجمة لسا تشتغل بدون كاش
  }
}

// نحافظ على عدد محدود من الحلقات المخزّنة (نحذف الأقدم لما نتجاوز الحد)
function pruneOldCacheEntries() {
  try {
    const keys = Object.keys(localStorage).filter(k => k.startsWith(TRANSLATE_CACHE_PREFIX));
    if (keys.length <= TRANSLATE_CACHE_MAX) return;
    // نحذف الزيادة من البداية (الأقدم غالباً أول من يضاف)
    keys.slice(0, keys.length - TRANSLATE_CACHE_MAX).forEach(k => localStorage.removeItem(k));
  } catch {}
}
...
export async function translateCues(cues, onProgress) {
  ...
  // ── تحقق من الكاش أولاً ──────────────────────────────────────────────────
  const cacheKey = getCacheKey(cues);
  if (cacheKey) {
    const cached = readCache(cacheKey);
    if (cached?.length === cues.length) {
      onProgress?.(cues.length, cues.length); // نعرض التقدم مكتمل فوراً
      return cached;
    }
  }
  ...
  // ── نخزن النتيجة بالكاش للمرة الجاية ────────────────────────────────────
  if (cacheKey) writeCache(cacheKey, translated);
